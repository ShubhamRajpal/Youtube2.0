...
import { ModeState, themeContext } from "../contexts/context";
...
const Header = () => {
  ...
  const { darkMode, setDarkMode } = ModeState();
  ...
  const getSearchSuggestions = async () => {
    ...
    const suggestionArr = suggestionsList
  ...
  return (
    <div
      className={`flex fixed justify-between py-4 sm:px-4 sm:py-2 h-15 z-30 w-screen lg:w-full ${
        darkMode ? "bg-[#0F0F0F]" : "bg-white"
      } `}>
      <div className="flex items-center w-[125px] sm:w-[135px]">
        <RxHamburgerMenu
          className={`cursor-pointer mx-1 sm:mx-3 ${darkMode && "text-white"} `}
          size={20}
        ...
        <div className="flex justify-start sm:pl-2 sm:justify-center items-center">
          ...
          <button
            className={`border border-gray-400 pt-[0] pb-[10] rounded-r-full px-2 ${
              darkMode ? "bg-[#ffffff14] border-zinc-700" : "bg-gray-200"
            }`}
            ...
            <svg
              ...
              <path
                ...
                fill={`${darkMode ? "white" : ""}`}></path>
            </svg>
          </button>
          <div
            className={`mx-4 p-2 rounded-full bg-gray-100 ${
              darkMode && "bg-[#ffffff14] text-white"
            } hidden sm:block md:block`}>
            <FaMicrophone size={20} className={`${darkMode && "text-black"}`} />
          </div>
        </div>
                ...
                suggestions.map((s) => (
                  <li
                    ...
                    className={`py-1 px-2 flex items-center gap-1 rounded-lg ${
                      darkMode ? "hover:bg-[#ffffff14]" : "hover:bg-gray-100"
                    }  cursor-pointer`}>
                    <svg
      ...
      <div className="flex ml-0 mr-1 sm:gap-2 md:gap-3 justify-end md:justify-end items-center">
        ...
        <MdOutlineVideoCall
          size={30}
          className={`${darkMode && "text-white"} hidden sm:block`}
        />
